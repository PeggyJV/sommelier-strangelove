...
export const isAPYEnabled = (config: ConfigProps) => {
  return (
    ...
    config.cellarNameKey === CellarNameKey.TURBO_STETH ||
    config.cellarNameKey === CellarNameKey.TURBO_OSETH ||
    config.cellarNameKey ===
      CellarNameKey.TURBO_STETH_STETH_DEPOSIT ||
    config.cellarNameKey ===
...
//need to update
  if (config.cellarNameKey === CellarNameKey.TURBO_OSETH) {
    return [
      {
        title: "7 Day Unbonding",
        amount: "1.1x SOMM",
...
export const estimatedApyValue = (config: ConfigProps) => {
  ...
  if (config.cellarNameKey === CellarNameKey.TEST_ARBITRUM_MULTI_ASSET_DEPOSIT) {
    return {
      value: 6.9,
      formatted: "6.9%",
    }
