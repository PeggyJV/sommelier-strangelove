...
const ROUTES = ["/", "/strategies", "/strategies/Alpha-stETH", "/strategies/Alpha-stETH/manage"]
...
async function run(url: string) {
  ...
  const opts: any = {
    ...
    throttling: { rttMs: 150, throughputKbps: 1638, cpuSlowdownMultiplier: 4, requestLatencyMs: 150, downloadThroughputKbps: 1638, uploadThroughputKbps: 750 },
  }
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUT_DIR, "lighthouse.json"), JSON.stringify(out, null, 2))
